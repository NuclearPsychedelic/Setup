...
--]]
    ...
    awful.key({ modkey, modkey2, }, "m", function() 
        awful.spawn.with_shell("xrandr --output eDP-1-1 --mode 1920x1080 --refresh 144 --rotate normal --output DP-0 --primary --mode 1920x1080 --refresh 144 --rotate normal --left-of eDP-1-1 && nitrogen --restore --set-zoom-fill ~/Pictures/Wallpapers") 
        end),
    -- single monitor:
    awful.key({ modkey, modkey2, "Shift" }, "m", function() 
        awful.spawn.with_shell("xrandr --output DP-0 --off --output eDP-1-1 --mode 1920x1080 --refresh 144 --rotate normal") 
        end),
...
--awful.spawn.with_shell("cd ~/.config/awesome && chmod +x dualmonitorDisplayPort.sh && xrandr --output DP-0 --off --output eDP-1-1 --auto && ./dualmonitorDisplayPort.sh &")
...
--[[
    Ok, I used your script and modified it a bit, since I found a way to use my monitor with my DisplayPort (to benefit from the higher refresh rate). Awesome starts just fine, but now it doesn't draw the top bar on my laptop monitor. What changed since yesterday: I started using DisplayPort, I made my dedicated GPU (Nvidia) the default using my distro's official documentation (Fedora) and I modified the script with the proper names for my displays. I should also mention that, since changing the default GPU, when plugged in, the external monitor is treated as a 
]]